...
import { addArticle, incrementViews, updateArticle } from "../slices/articlesSlice";
...
export const localStorageMiddleware: Middleware = (store) => (next) => (action) => {
  if (incrementViews.match(action)) {
    ...
    const currentArticle = state.articles.articles.find(article => article.id === action.payload);
